...
static void out_channel_value_callback(CSOUND *csound, const char *name, void *valp, const void *channelType) {
    ...
    // update the struct at the current outbuf write index and update it
    // the below makes csound segfault sometimes, problem is that this callback really runs in the audio thread
    // note: surrounding in a critical_enter just crashes
    //strcpy( x->out_msg_buf[ x->out_msg_write_index ].name, name);
    //x->out_msg_buf[ x->out_msg_write_index ].value = out_value;
    //x->out_msg_write_index = x->out_msg_write_index < x->out_msg_max ? x->out_msg_write_index + 1 : 0;
    //x->out_msg_count++;
    ...
    //post("  - scheduling x->out_msg_clock, write index now: %i", x->out_msg_write_index);
...
static void csound6_out_msg_cb(t_csound6 *x){
    ...
    atom_setsym( &out_atoms[1], gensym("test") );
    atom_setfloat( &out_atoms[2], 1.234 );
